...
w <- pred.qrnn2 <- vector("list", length(tau))

for(i in seq_along(tau)){
  w[[i]] <- qrnn2.fit(x=X_train, y=Y_train, n.hidden=1, n.hidden2=10,
                      tau=tau[i], iter.max=1000, n.trials=1, penalty=1)
}
train_end <- Sys.time()

inference_start <- Sys.time()
pred.qrnn2 <- vector("list", length(tau))
for(i in seq_along(tau)){
  pred.qrnn2[[i]] <- qrnn2.predict(X_test, w[[i]])
}
...
for (i in seq_along(pred.qrnn2)) {
  results[[i]] <- pred.qrnn2[[i]]
}
qr2nn <- do.call(cbind, results)

#matplot( cbind(qr2nn[,1],qr2nn[,19] ,Y_test), type = "l", lty = 1, col = c("red","red", "blue"))
#qrnnpred <- as.data.frame(qr2nn)
#write.xlsx(qrnnpred,file = "C:/Users/ym_ya/Desktop/code and data/tvfemd_R2CMSE/tsub1/qr2nnpred1.xlsx")